...
import WalletDisconnectModal from '../Modal/WalletDisconnectModal';
...
const Nav: React.FC = () => {
  ...
  return (
    <>
      <nav className="w-full px-12 xl:px-18 2xl:px-48 3xl:px-52 py-4 xl:py-6 absolute top-0 left-0 z-20 bg-black xl:bg-transparent">
        ...
        <div className="w-full max-w-screen-2xl m-auto flex justify-between items-center">
          ...
          {width < MOBILE_BREAKPOINT && (
            <Hamburger toggled={isHamburgerOpen} toggle={setIsHamburgerOpen} color="white" />
          )}
          {isHamburgerOpen && (
            <HamburgerMenu
          ...
          <div className="hidden xl:flex flex-row gap-x-12">
            ...
            {address ? (
              <button
                className="rounded-2xl text-xl font-bold px-4 py-2 text-black bg-cyan"
      ...
      <WalletDisconnectModal show={isWalletDisconnectModalOpen} onClose={() => setIsWalletDisconnectModalOpen(false)} />
    </>
  );
};
